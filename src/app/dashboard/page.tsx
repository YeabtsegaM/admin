...
import ShopOwnersSection from '@/components/sections/ShopOwnersSection';
...
import GameResultSection from '@/components/sections/GameResultSection';
import SlipDetailSection from '@/components/sections/SlipDetailSection';
...
export default function DashboardPage() {
  ...
  const handleSectionChange = (section: string) => {
    ...
    // Navigate to direct routes instead of query parameters
    if (section === 'shop-owners') router.push('/shopowners');
    else if (section === 'shops') router.push('/shops');
    else if (section === 'cashiers') router.push('/cashiers');
    ...
    // For dashboard, stay on current page
  };
        ...
        <div className={`flex-1 overflow-hidden transition-all duration-300 bg-gray-50 ${
          ...
          <main className="h-full overflow-y-auto">
            {activeSection === 'dashboard' && (
              <DashboardOverview ref={dashboardRef} />
            )}
            {activeSection === 'shop-owners' && (
              <ShopOwnersSection />
            )}
            {activeSection === 'shops' && (
              <ShopsSection />
            ...
            {activeSection === 'game-result' && (
              <GameResultSection />
            )}
            {activeSection === 'slip-detail' && (
              <SlipDetailSection />
            )}
          </main>
        </div>
